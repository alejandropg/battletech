...
- **A wire-crossing `session`/`rules` sealed type needs `@SerialName` on every variant**: `WireJson` uses `classDiscriminator = "type"`, and an unannotated variant's discriminator is its fully-qualified class name — moving it between packages silently changes the wire format. Bump `PROTOCOL_VERSION` (`network/wire/Messages.kt`) on any non-additive change to `GameCommand`/`GameEvent`/`CommandRejection`/`RuleRejection`.
