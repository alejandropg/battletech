...
- **Every player is a client — locality is an adapter, not a branch**: a seat at this terminal and a seat across the internet both reach the session through `transport/`'s connection port — `InMemoryConnection` for local, `JsonLineConnection` for remote. `GameServer` cannot tell them apart, and it is deliberately NOT a `GameSession` (no local seat to be one for). Hot-seat = a server + two `connectLocal()` clients + no socket; `--host` = one `connectLocal()` + a `SocketAcceptor`. `main()` is the only place that knows which mode ran — do not add a branch anywhere else that asks "is this hot-seat?", and do not give a local player a private path to the session. That is what this design exists to prevent: the last one let the seat check drift out of sync with itself. Rationale: `docs/architecture.md`.
