...
- **`tenter/`** — a standalone terminal-UI toolkit over [Mordant](https://github.com/ajalt/mordant): screen buffer/diffing renderer/color-role palettes (`screen/`), layout decorators and widgets (`view/`), the panel framework (`panel/`), chrome input mappings and terminal event flows (`input/`, `terminal/`), a Nerd Font glyph catalog (`icons/`). Knows nothing about BattleTech — see the invariant below — so it is the one module meant to be lifted out into its own library later.
