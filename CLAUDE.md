...
- **Subscription is canonical**: `session.subscribe(listener)` is how remote/web clients will receive `GameEvent`s — session-wide and unfiltered, since the game is open-information by design (see `GameServer.snapshot`'s KDoc). `CommandResult.Accepted.events` is a synchronous courtesy returned to the submitter; do not rely on it for cross-player notification.
