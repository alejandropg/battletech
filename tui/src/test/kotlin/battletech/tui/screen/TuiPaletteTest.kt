...
/**
 * Verifies [TuiPalette] against the guarantees laid out in the color-theme plan: WCAG contrast
 * for truecolor and ANSI-256 themes (their sRGB values are fixed and reproducible), distinctness
 * only for ANSI-16 themes (codes `0..15` are terminal-configurable, so their sRGB — and therefore
 * contrast — is unknowable at build time; do not add a contrast assertion for [TuiTheme.DARK_16]
 * or [TuiTheme.LIGHT_16]), and the reduced-theme design decision that every `TERRAIN_*_BG` role
...
internal class TuiPaletteTest {
     ...
     */
    private val generalRoles = Color.entries.filterNot {
        it == Color.DEFAULT || it in TERRAIN_FILLS || it in TERRAIN_ICONS || it in BADGE_BACKGROUNDS ||
            it == Color.ELEVATION_BADGE_FG || it in SUBTLE_ROLES
    }
    ...
    fun `truecolor default surfaces match the exact authored hex values`() {
        ...
        assertThat(dark.foreground(Color.DEFAULT)).isEqualTo(rgb(0xDD, 0xE2, 0xE5))
        assertThat(dark.background(Color.DEFAULT)).isEqualTo(rgb(0x10, 0x14, 0x18))
    ...
    fun `ansi256 default surfaces match the exact authored indices`() {
        val dark256 = TuiPalette.forTheme(TuiTheme.DARK_256)
        assertThat(dark256.foreground(Color.DEFAULT)).isEqualTo(PaletteColor.Indexed(253))
        assertThat(dark256.background(Color.DEFAULT)).isEqualTo(PaletteColor.Indexed(233))

        val light256 = TuiPalette.forTheme(TuiTheme.LIGHT_256)
        assertThat(light256.foreground(Color.DEFAULT)).isEqualTo(PaletteColor.Indexed(235))
        assertThat(light256.background(Color.DEFAULT)).isEqualTo(PaletteColor.Indexed(255))
    }

    @Test
    fun `ansi16 default surfaces match the exact authored codes`() {
        val dark16 = TuiPalette.forTheme(TuiTheme.DARK_16)
        assertThat(dark16.foreground(Color.DEFAULT)).isEqualTo(PaletteColor.Basic(37))
        assertThat(dark16.background(Color.DEFAULT)).isEqualTo(PaletteColor.Basic(30))

        val light16 = TuiPalette.forTheme(TuiTheme.LIGHT_16)
        assertThat(light16.foreground(Color.DEFAULT)).isEqualTo(PaletteColor.Basic(30))
        assertThat(light16.background(Color.DEFAULT)).isEqualTo(PaletteColor.Basic(97))
    }
    ...
    fun `every theme resolves every non-default role in its own PaletteColor space`() {
        for (theme in TuiTheme.entries) {
            val palette = TuiPalette.forTheme(theme)
            for (role in Color.entries) {
    ...
    fun `truecolor- light woods is brighter than heavy woods, shallow water is brighter than deep water`() {
        for (theme in truecolorThemes) {
            val palette = TuiPalette.forTheme(theme)
            val light = luminance(palette.background(Color.TERRAIN_WOODS_LIGHT_BG))
            val heavy = luminance(palette.background(Color.TERRAIN_WOODS_HEAVY_BG))
            assertThat(light).describedAs("$theme light vs heavy woods luminance").isGreaterThan(heavy)

            val shallow = luminance(palette.background(Color.TERRAIN_WATER_SHALLOW_BG))
            val deep = luminance(palette.background(Color.TERRAIN_WATER_DEEP_BG))
    ...
    fun `truecolor and ansi256- elevation badge foreground clears 4point5 to 1 against every badge tier`() {
        for (theme in truecolorThemes + ansi256Themes) {
            val palette = TuiPalette.forTheme(theme)
            val fg = palette.foreground(Color.ELEVATION_BADGE_FG)
    ...
    fun `truecolor and ansi256- every general role clears 4point5 to 1 against the default background, except DISABLED at 3 to 1`() {
        for (theme in truecolorThemes + ansi256Themes) {
            val palette = TuiPalette.forTheme(theme)
            val defaultBg = luminance(palette.background(Color.DEFAULT))
            for (role in generalRoles) {
                val required = if (role == Color.DISABLED) 3.0 else 4.5
    ...
    fun `truecolor and ansi256- default foreground clears 4point5 to 1 against default background`() {
        for (theme in truecolorThemes + ansi256Themes) {
            val palette = TuiPalette.forTheme(theme)
            val ratio = contrast(luminance(palette.foreground(Color.DEFAULT)), luminance(palette.background(Color.DEFAULT)))
    ...
    fun `truecolor and ansi256- decorative BOARD_BORDER is merely distinct from every terrain fill, not required to pop`() {
        ...
        for (theme in truecolorThemes + ansi256Themes) {
            val palette = TuiPalette.forTheme(theme)
            val fg = palette.foreground(Color.BOARD_BORDER)
            for (fill in TERRAIN_FILLS) {
    ...
    fun `ansi256- the water and rough icon colors are mutually distinct from each other and from woods`() {
        ...
        for (theme in ansi256Themes) {
            val palette = TuiPalette.forTheme(theme)
            val light = palette.foreground(Color.TERRAIN_WOODS_LIGHT_ICON)
            val heavy = palette.foreground(Color.TERRAIN_WOODS_HEAVY_ICON)
            val water = palette.foreground(Color.TERRAIN_WATER_ICON)
            val rough = palette.foreground(Color.TERRAIN_ROUGH_ICON)
    ...
    fun `ansi16- roles the player must tell apart do not share a code`() {
        for (theme in ansi16Themes) {
            val palette = TuiPalette.forTheme(theme)
            assertNotEquals(palette.foreground(Color.PLAYER_1), palette.foreground(Color.PLAYER_2), "$theme: PLAYER_1 vs PLAYER_2")
            val moves = listOf(Color.MOVE_WALK, Color.MOVE_RUN, Color.MOVE_JUMP).map { palette.foreground(it) }
            assertThat(moves.toSet()).describedAs("$theme move colors").hasSize(3)
            assertNotEquals(palette.foreground(Color.TARGET_VALID), palette.foreground(Color.TARGET_SELECTED), "$theme: TARGET_VALID vs TARGET_SELECTED")
            assertNotEquals(palette.foreground(Color.BOARD_ACTIVE), palette.foreground(Color.BOARD_BORDER), "$theme: BOARD_ACTIVE vs BOARD_BORDER")
            assertNotEquals(palette.foreground(Color.TERRAIN_WATER_ICON), palette.foreground(Color.TERRAIN_ROUGH_ICON), "$theme: water vs rough icon")
            assertNotEquals(palette.foreground(Color.TERRAIN_WOODS_LIGHT_ICON), palette.foreground(Color.TERRAIN_WATER_ICON), "$theme: woods vs water icon")
            assertNotEquals(palette.foreground(Color.TERRAIN_WOODS_LIGHT_ICON), palette.foreground(Color.TERRAIN_ROUGH_ICON), "$theme: woods vs rough icon")
            val badges = BADGE_BACKGROUNDS.map { palette.background(it) }
            assertThat(badges.toSet()).describedAs("$theme badge tiers").hasSize(BADGE_BACKGROUNDS.size)
        }
    }
    ...
    fun `reduced themes- every terrain fill role resolves to the default background`() {
        for (theme in reducedThemes) {
            val palette = TuiPalette.forTheme(theme)
            val defaultBg = palette.background(Color.DEFAULT)
            for (fill in TERRAIN_FILLS) {
    ...
    private companion object {
        ...
        private val SUBTLE_ROLES = listOf(Color.TEXT_SUBTLE, Color.BOARD_BORDER)
        ...
        private val TERRAIN_ICONS = listOf(
            Color.TERRAIN_WOODS_LIGHT_ICON,
            Color.TERRAIN_WOODS_HEAVY_ICON,
            Color.TERRAIN_WATER_ICON,
            Color.TERRAIN_ROUGH_ICON,
        )
        private val BADGE_BACKGROUNDS = listOf(
            Color.ELEVATION_1_BADGE_BG,
            Color.ELEVATION_2_BADGE_BG,
            Color.ELEVATION_HIGH_BADGE_BG,
        )
        private val CRITICAL_BOARD_FOREGROUNDS = listOf(
            Color.TEXT_PRIMARY, Color.MOVE_WALK, Color.MOVE_RUN, Color.MOVE_JUMP,
            Color.PLAYER_1, Color.PLAYER_2, Color.ATTACK_RANGE, Color.BOARD_ACTIVE,
            Color.LINE_OF_SIGHT, Color.TARGET_VALID, Color.TARGET_SELECTED,
        )
        private val ICON_TO_FILLS = mapOf(
            Color.TERRAIN_WOODS_LIGHT_ICON to listOf(Color.TERRAIN_WOODS_LIGHT_BG),
            Color.TERRAIN_WOODS_HEAVY_ICON to listOf(Color.TERRAIN_WOODS_HEAVY_BG),
            Color.TERRAIN_WATER_ICON to listOf(Color.TERRAIN_WATER_SHALLOW_BG, Color.TERRAIN_WATER_DEEP_BG),
            Color.TERRAIN_ROUGH_ICON to listOf(Color.TERRAIN_ROUGH_BG),
        )
