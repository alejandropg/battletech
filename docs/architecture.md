...
- **`tactical/`** (`battletech.tactical.*`) — delivery-agnostic: every delivery (TUI, `network`, any future web UI) consumes it through the same public surface. `attack/` (incl. `physical/`, `weapon/`), `dice/`, `heat/`, `io/` (`ResourceOrFileLoader`), `model/` (core state plus `map/`'s catalog/loading and `game/`'s validated starting-game definitions), `movement/`, `query/`, `rules/`, `session/`, and `unit/`. Game loading returns a complete `GameState`; launchers do not separately construct maps and rosters.
...
## Game, map, and theme loading share one loader

Game, map, and theme loading share `battletech.tactical.io.ResourceOrFileLoader<T>`. `GameStateLoader`, `GameMapLoader`, and `ThemeLoader` each supply their domain-specific decoder/builder and get the same "existing path is authoritative, otherwise load `<dir>/<spec>.json`, with names from `<dir>/index.json`" behavior. The generic loader remains domain-free.

`GameMapCatalog` adds the startup registration policy that raw map loading does not own: it exposes packaged maps by indexed name, eagerly loads each external `--map` path under its filename-derived name, rejects every built-in/external collision, and resolves the game file's `map` field. `resolveGame` is the outer seam: it decodes the starting-game file, validates the roster against that catalog, constructs pristine `CombatUnit`s from `MechModels`, and returns the complete `GameState`. The on-disk DTOs stay internal. Packaging detail for `game/`, `map/`, and `theme/` lives in `docs/build.md`.
...
The board (`GameMap`) is a join-time payload, not something a client picks: it arrives once inside `ServerMessage.JoinAccepted`. `--game` selects the starting definition and repeatable `--map` options register external maps only on modes that build the authoritative state (`Local`/`Host`/`Server`), never on `join`. Every seat still runs the same local-map identification check after the host supplies the board.
