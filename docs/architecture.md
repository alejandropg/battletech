...
Unit, map, mech, and theme loading share `battletech.tactical.io.ResourceOrFileLoader<T>`. `UnitFileLoader`, `GameMapLoader`, `MechModelLoader`, and `ThemeLoader` each supply their domain-specific decoder/builder and reuse its resource/file error handling. The generic loader remains domain-free.

`GameMapCatalog`, `MechModelCatalog`, and `UnitCatalog` each add their own content's registration policy — `GameMapCatalog`/`UnitCatalog` select one whole registered entry by name, `MechModelCatalog` eagerly flattens every collection named by `mech/index.json` plus repeatable external `--add-mech` paths into one variant registry, validating models and rejecting duplicate variants without overrides. `battletech.tactical.model.content.ContentCatalog` is the deep module over all three: it is the one seam both launching (`ContentCatalog.resolveGame`, which resolves the selected map and unit collection, constructs pristine `CombatUnit`s from the selected variants, and returns the complete `GameState`) and listing (`ContentCatalog.listing`, backing `--list-maps`/`--list-mechs`/`--list-units`) go through, so they can never disagree about what is registered or valid. The on-disk DTOs stay internal. Packaging detail lives in `docs/build.md`.
...
The board (`GameMap`) and match mech definitions are join-time payloads, not content a client picks. One `ServerMessage.JoinAccepted` carries a complete `MatchBootstrap`: the models, map, initial projected units and positions, turn state, and redacted log. The connection decoder installs the embedded model catalog before decoding compact variant references in the snapshot; later pushes keep encoding each model as only its variant and never resend immutable match content. `--map`/`--unit` (SELECTING a board/roster) therefore exist only on modes that build authoritative state (`hot-seat`/`host`/`server`), never on `join` — but `--add-map`/`--add-mech`/`--add-unit` (REGISTERING external content) are root options valid on every mode, `join` included: a joining seat can register content purely so its local-drift comparison has more than the packaged catalog to compare the host's supplied map and mech definitions against (`ContentCatalog.mapMatcher`/`mechFinder`, wired into `ClientGameSession.connect`). Differences produce local informational warnings, never rejection or substitution: host content remains authoritative, and content unavailable locally is accepted silently.
