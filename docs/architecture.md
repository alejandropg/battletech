...
- **`tactical/`** (`battletech.tactical.*`) — delivery-agnostic: every delivery (TUI, `network`, any future web UI) consumes it through the same public surface. `attack/` (incl. `physical/`, `weapon/`), `dice/`, `heat/`, `io/` (`ResourceOrFileLoader`), `model/` (core state plus `map/`, `game/`, and `mech/` content loading), `movement/`, `query/`, `rules/`, `session/`, and `unit/`. Game loading returns a complete `GameState`; launchers do not separately construct maps and rosters.
...
## Game, map, mech, and theme loading share one loader

Game, map, mech, and theme loading share `battletech.tactical.io.ResourceOrFileLoader<T>`. `GameStateLoader`, `GameMapLoader`, `MechModelLoader`, and `ThemeLoader` each supply their domain-specific decoder/builder and reuse its resource/file error handling. The generic loader remains domain-free.

`GameMapCatalog` adds the map registration policy; `MechModelCatalog` is the corresponding deep module for mech content. It eagerly combines every collection named by `mech/index.json` with repeatable external `--mech` paths, validates models, and rejects duplicate variants without overrides. `resolveGame` receives both catalogs, constructs pristine `CombatUnit`s from the selected variants, and returns the complete `GameState`. The on-disk DTOs stay internal. Packaging detail lives in `docs/build.md`.
...
The board (`GameMap`) and match mech definitions are join-time payloads, not content a client picks. One `ServerMessage.JoinAccepted` carries a complete `MatchBootstrap`: the models, map, initial projected units and positions, turn state, and redacted log. The connection decoder installs the embedded model catalog before decoding compact variant references in the snapshot; later pushes keep encoding each model as only its variant and never resend immutable match content. `--game`, repeatable `--map`, and repeatable `--mech` therefore exist only on modes that build authoritative state (`Local`/`Host`/`Server`), never on `join`. Every seat compares the supplied map and any same-variant mech definitions with its packaged local content. Differences produce local informational warnings, never rejection or substitution: host content remains authoritative, and content unavailable locally is accepted silently.
